...
const AddProductPage = () => {
    ...
    const [productToEdit, setProductToUpdate] = useState(null);
    ...
    useEffect(() => {
        const getArtist = async () => {
            try {
                if (user.id) {
                    const response = await axios.get(`http://localhost:5000/api/artist/${user.id}`);
                    setArtistName(response.data.artist.name);
                } else {
    ...
    const ProductAction = async (data) => {
        try {
            if (!productToEdit) {
                await dispatch(addProducts({ data, artistName })).unwrap();
                SuccessToast("Product has been added.");
            } else {
                await dispatch(updateProducts({ data, artistName, id })).unwrap(); // Using id directly
                SuccessToast("Product has been updated.");
            }
        } catch (error) {
    ...
    return (
        <div className="flex flex-col items-center justify-center gap-[20px]">
            ...
            <form className="flex flex-col gap-[20px] my-[20px]" onSubmit={handleSubmit(ProductAction)}>
                <div className="grid grid-rows-1 grid-cols-2 gap-[30px] font-slab">
                    <div className="flex flex-col gap-[20px]">
                        <label>Product Name</label>
                        <Input
                ...
                <DashboardActionButton>{productToEdit ? "Update Product" : "Add Product"}</DashboardActionButton>
            </form>
        </div>
    );
};
